...
# Mihomo Smart内核及模型更新脚本
# 更新mihomo Smart内核或LightGBM模型并重启Nikki服务
...
# 全局变量
NIKKI_DIR="/etc/nikki"
MODEL_DIR="${NIKKI_DIR}/run"
MODEL_PATH="${MODEL_DIR}/Model.bin"
MODEL_BACKUP_PATH="${MODEL_PATH}.bak"
MODEL_BASE_URL="https://github.com/vernesong/mihomo/releases/download/LightGBM-Model"
CORE_DIR="/usr/bin"
...
SCRIPT_VERSION="1.1.7" # 脚本版本号
AUTO_UPDATE_SCHEDULE="0 3 * * *"
AUTO_UPDATE_VARIANT_CHOICE="auto"
AUTO_UPDATE_LOG="/tmp/smartcore_update.log"
SMART_COMMAND_PATH="/usr/bin/smart"
GITHUB_ACCELERATOR="${SMARTCORE_GITHUB_ACCELERATOR:-https://cdn.gh-proxy.org/}"
...
UPDATE_VARIANT_CHOICE="${SMARTCORE_UPDATE_VARIANT_CHOICE:-auto}"
...
download_file() {
  ...
  if command -v curl >/dev/null 2>&1 &&
     curl --fail -L --progress-bar -o "$DOWNLOAD_OUTPUT" "$DOWNLOAD_URL"; then
    return 0
  fi
  ...
  if [ -n "$ACCELERATED_URL" ]; then
    ...
    if command -v curl >/dev/null 2>&1 &&
       curl --fail -L --progress-bar --connect-timeout "$GITHUB_ACCELERATOR_CONNECT_TIMEOUT" --max-time "$GITHUB_ACCELERATOR_MAX_TIME" -o "$DOWNLOAD_OUTPUT" "$ACCELERATED_URL"; then
      return 0
    fi
...
# 安装smart快捷命令
install_smart_command() {
  get_script_path || return 1

  if [ "$SCRIPT_PATH" = "$SMART_COMMAND_PATH" ]; then
    chmod 755 "$SMART_COMMAND_PATH" 2>/dev/null || true
    return 0
  fi

  if [ -e "$SMART_COMMAND_PATH" ] && [ ! -f "$SMART_COMMAND_PATH" ]; then
    log "警告: ${SMART_COMMAND_PATH} 已存在且不是普通文件，无法安装smart快捷命令"
    return 1
  fi

  if [ -f "$SMART_COMMAND_PATH" ] && cmp -s "$SCRIPT_PATH" "$SMART_COMMAND_PATH" 2>/dev/null; then
    chmod 755 "$SMART_COMMAND_PATH" 2>/dev/null || true
    return 0
  fi

  if cp "$SCRIPT_PATH" "$SMART_COMMAND_PATH" 2>/dev/null &&
     chmod 755 "$SMART_COMMAND_PATH" 2>/dev/null; then
    log "已安装快捷命令: smart"
    return 0
  fi

  log "警告: 无法安装smart快捷命令，请确认当前用户有写入 ${SMART_COMMAND_PATH} 的权限"
  return 1
}
...
normalize_variant_choice() {
  VARIANT_CHOICE_INPUT="$1"

  case "$VARIANT_CHOICE_INPUT" in
    ""|"auto"|"AUTO"|"Auto")
      echo "auto"
      ;;
    [1-6])
      echo "$VARIANT_CHOICE_INPUT"
      ;;
    *)
      return 1
      ;;
...
# 获取架构对应的版本变体显示名
get_variant_choice_display() {
  DISPLAY_CHOICE=$(normalize_variant_choice "$1" 2>/dev/null || echo "auto")
  DISPLAY_ARCH="${2:-$(uname -m)}"
  ...
  case "$DISPLAY_ARCH" in
    "x86_64"|"amd64")
      case "$DISPLAY_CHOICE" in
        1) echo "标准版本 (mihomo-linux-amd64)" ;;
        2) echo "兼容版本 (mihomo-linux-amd64-compatible)" ;;
        3) echo "Go 1.20版本 (mihomo-linux-amd64-go120)" ;;
        4) echo "Go 1.23版本 (mihomo-linux-amd64-go123)" ;;
        5) echo "兼容Go 1.20版本 (mihomo-linux-amd64-compatible-go120)" ;;
        6) echo "兼容Go 1.23版本 (mihomo-linux-amd64-compatible-go123)" ;;
        *) echo "自动推荐" ;;
      esac
      ;;
    "i386"|"i686"|"x86")
      case "$DISPLAY_CHOICE" in
        1) echo "标准版本 (mihomo-linux-386)" ;;
        2) echo "Go 1.20版本 (mihomo-linux-386-go120)" ;;
        3) echo "Go 1.23版本 (mihomo-linux-386-go123)" ;;
        4) echo "Softfloat版本 (mihomo-linux-386-softfloat)" ;;
        *) echo "自动推荐" ;;
      esac
      ;;
    *)
...
apply_variant_choice() {
  VARIANT_CHOICE=$(normalize_variant_choice "$1" 2>/dev/null || echo "")

  if [ -z "$VARIANT_CHOICE" ] || [ "$VARIANT_CHOICE" = "auto" ]; then
    return 0
  fi
  ...
  case "$ARCH_NAME" in
    "amd64")
      case "$VARIANT_CHOICE" in
        1) USE_COMPATIBLE=""; USE_GO_VERSION="" ;;
        2) USE_COMPATIBLE="-compatible"; USE_GO_VERSION="" ;;
        3) USE_COMPATIBLE=""; USE_GO_VERSION="-go120" ;;
        4) USE_COMPATIBLE=""; USE_GO_VERSION="-go123" ;;
...
get_auto_update_entry() {
  AUTO_UPDATE_COMMAND=$(get_auto_update_command) || return 1
  AUTO_UPDATE_VARIANT_CHOICE=$(normalize_variant_choice "$AUTO_UPDATE_VARIANT_CHOICE") || return 1
  printf '%s %s --auto --variant-choice %s >> %s 2>&1\n' "$AUTO_UPDATE_SCHEDULE" "$AUTO_UPDATE_COMMAND" "$AUTO_UPDATE_VARIANT_CHOICE" "$AUTO_UPDATE_LOG"
}
...
get_auto_update_cron_line() {
  ...
  CRON_LINE=$(crontab -l 2>/dev/null | grep -F "$SMART_COMMAND_PATH --auto" | tail -n 1)
  if [ -z "$CRON_LINE" ] && [ -n "$SCRIPT_PATH" ]; then
    CRON_LINE=$(crontab -l 2>/dev/null | grep -F "$SCRIPT_PATH --auto" | tail -n 1)
  fi

  [ -n "$CRON_LINE" ] || return 1
  echo "$CRON_LINE"
}
...
# 从计划任务中提取自动更新版本变体
parse_auto_update_variant_choice() {
  CRON_LINE="$1"
  NEXT_IS_VARIANT=""

  for CRON_FIELD in $CRON_LINE; do
    if [ "$NEXT_IS_VARIANT" = "1" ]; then
      normalize_variant_choice "$CRON_FIELD" 2>/dev/null || echo "auto"
      return 0
    fi
...
# 从现有计划任务中加载自动更新版本变体
load_auto_update_variant_choice() {
  CRON_LINE=$(get_auto_update_cron_line 2>/dev/null || echo "")

  if [ -n "$CRON_LINE" ]; then
    AUTO_UPDATE_VARIANT_CHOICE=$(parse_auto_update_variant_choice "$CRON_LINE")
    return 0
  fi
...
# 让用户输入自动更新版本变体
prompt_auto_update_variant_choice() {
  PROMPT_ARCH=$(uname -m)
  CURRENT_VARIANT=$(get_variant_choice_display "$AUTO_UPDATE_VARIANT_CHOICE" "$PROMPT_ARCH")
  ...
  case "$PROMPT_ARCH" in
    "x86_64"|"amd64")
      ...
      printf "请输入选项 [1-6] (当前: %s，回车保持不变): " "$CURRENT_VARIANT"
      read -r INPUT_VARIANT_CHOICE
      [ -z "$INPUT_VARIANT_CHOICE" ] && return 0

      case "$INPUT_VARIANT_CHOICE" in
        [1-6])
          AUTO_UPDATE_VARIANT_CHOICE="$INPUT_VARIANT_CHOICE"
          return 0
          ;;
      esac
      ;;
    "i386"|"i686"|"x86")
      ...
      printf "请输入选项 [1-4] (当前: %s，回车保持不变): " "$CURRENT_VARIANT"
      read -r INPUT_VARIANT_CHOICE
      [ -z "$INPUT_VARIANT_CHOICE" ] && return 0
...
is_auto_update_enabled() {
  get_auto_update_cron_line >/dev/null 2>&1
}
...
enable_auto_update() {
  ...
  load_auto_update_variant_choice >/dev/null 2>&1 || true
  ...
  log "自动更新版本变体: $(get_variant_choice_display "$AUTO_UPDATE_VARIANT_CHOICE")"
...
detect_arch() {
  ...
  case "$ARCH" in
    "x86_64"|"amd64")
      ...
      if [ -x "$(command -v ldd)" ]; then
        GLIBC_VERSION=$(ldd --version 2>/dev/null | head -n1 | grep -o '[0-9]\+\.[0-9]\+$' || echo "")
      ...
      if [ -t 0 ] && [ -z "$AUTO_MODE" ]; then
        ...
        apply_variant_choice "$variant_choice" || {
          log "错误: 无效的版本变体选项"
          exit 1
        }
      fi
  ...
  UPDATE_VARIANT_CHOICE=$(normalize_variant_choice "$UPDATE_VARIANT_CHOICE" 2>/dev/null || echo "")
  if [ -z "$UPDATE_VARIANT_CHOICE" ]; then
    log "错误: 自动更新版本变体参数无效"
    exit 1
  fi

  if [ -n "$AUTO_MODE" ] && [ "$UPDATE_VARIANT_CHOICE" != "auto" ]; then
    apply_variant_choice "$UPDATE_VARIANT_CHOICE" || {
      log "错误: 当前架构不支持该版本变体选项"
      exit 1
    }
    log "使用版本变体: $(get_variant_choice_display "$UPDATE_VARIANT_CHOICE" "$ARCH")"
  fi
...
# 下载并更新模型；子进程单独清理临时文件，不影响主菜单的退出处理
update_model() (
  case "$1" in
    1) MODEL_FILENAME="Model.bin" ;;
    2) MODEL_FILENAME="Model-middle.bin" ;;
    3) MODEL_FILENAME="Model-large.bin" ;;
    *)
      log "错误: 无效的模型选项"
      return 1
      ;;
  ...
  if [ ! -x "$SERVICE_SCRIPT" ]; then
    log "错误: 未找到可执行的Nikki服务脚本: ${SERVICE_SCRIPT}"
    return 1
  fi

  if { [ -e "$MODEL_PATH" ] && [ ! -f "$MODEL_PATH" ]; } ||
     { [ -e "$MODEL_BACKUP_PATH" ] && [ ! -f "$MODEL_BACKUP_PATH" ]; }; then
    log "错误: 模型或备份路径已存在且不是普通文件"
    return 1
  fi

  mkdir -p "$MODEL_DIR" || {
    log "错误: 无法创建模型目录: ${MODEL_DIR}"
    return 1
  }

  # 在目标目录暂存，下载完成后通过同一文件系统内的重命名替换
  MODEL_TEMP_FILE=$(mktemp "${MODEL_DIR}/.Model.bin.XXXXXX") || {
    log "错误: 无法创建模型临时文件，请检查目录权限和剩余空间"
    return 1
  }
  trap 'rm -f "$MODEL_TEMP_FILE"' EXIT
  trap 'exit 130' INT
  trap 'exit 143' TERM

  log "开始下载模型: ${MODEL_FILENAME}"
  if ! download_file "${MODEL_BASE_URL}/${MODEL_FILENAME}" "$MODEL_TEMP_FILE"; then
    log "错误: 模型下载失败，原模型未修改"
    return 1
  fi

  # 上游.bin文件使用LightGBM文本格式，检查头部和树结束标记
  # 避免将空文件、错误页面或明显不完整的下载覆盖为当前模型
  if [ ! -s "$MODEL_TEMP_FILE" ] ||
     [ "$(head -n 1 "$MODEL_TEMP_FILE")" != "tree" ] ||
     ! grep -q '^version=v' "$MODEL_TEMP_FILE" ||
     ! grep -q '^Tree=0$' "$MODEL_TEMP_FILE" ||
     ! grep -q '^end of trees$' "$MODEL_TEMP_FILE"; then
    log "错误: 下载内容不是有效的LightGBM模型或文件不完整，原模型未修改"
    return 1
  fi

  if [ -f "$MODEL_PATH" ] && cmp -s "$MODEL_TEMP_FILE" "$MODEL_PATH"; then
    log "当前模型与所选 ${MODEL_FILENAME} 相同，无需更新"
    return 0
  fi

  chmod 644 "$MODEL_TEMP_FILE" || {
    log "错误: 无法设置模型文件权限，原模型未修改"
    return 1
  }

  if [ -f "$MODEL_PATH" ]; then
    if ! cp "$MODEL_PATH" "$MODEL_BACKUP_PATH"; then
      log "错误: 备份原模型失败，原模型未修改"
      return 1
    fi
    log "原模型已备份至: ${MODEL_BACKUP_PATH}"
  fi

  if ! mv -f "$MODEL_TEMP_FILE" "$MODEL_PATH"; then
    log "错误: 安装新模型失败，原模型未修改"
    return 1
  fi

  log "模型已安装至: ${MODEL_PATH} (${MODEL_FILENAME})"
  log "重启Nikki服务..."
  if ! "$SERVICE_SCRIPT" restart; then
    log "错误: 模型已更新，但Nikki重启失败，请检查服务后手动重启"
    return 1
  fi

  echo -e "${GREEN}Model.bin更新成功！${NC}"
  return 0
)
...
manage_model_update() {
  clear
  echo "==========================================="
  echo "        Nikki Model.bin 模型更新"
  echo "==========================================="
  echo "1. 默认模型 (Model.bin)"
  echo "2. 中型模型 (Model-middle.bin)"
  echo "3. 大型模型 (Model-large.bin)"
  echo "0. 返回主菜单"
  echo
  echo "所选模型统一安装为: ${MODEL_PATH}"
  echo "更新前备份旧模型，更新后重启Nikki服务"
  printf "请选择模型 [0-3]: "
  read -r model_choice || return 0

  case "$model_choice" in
    0) return 0 ;;
    [1-3])
      if update_model "$model_choice"; then
        :
      else
        log "模型更新未完成，请查看上方错误信息"
      fi
      ;;
    *) echo "无效的选择，请输入0-3" ;;
  esac
...
manage_auto_update() {
  ...
  echo "版本变体: $(get_variant_choice_display "$AUTO_UPDATE_VARIANT_CHOICE")"
  echo "脚本路径: ${SCRIPT_PATH:-未知}"
  echo "快捷命令: smart"
  echo "日志路径: ${AUTO_UPDATE_LOG}"
  echo
  echo "1. 启用或更新时间/版本变体自动更新任务"
  echo "2. 关闭自动更新任务"
  echo "0. 返回主菜单"
  echo "==========================================="
...
show_menu() {
  ...
  echo -e "- Nikki模型路径: ${MODEL_PATH}"
  echo -e "- 更新前会自动备份当前内核"
  echo -e "- 如更新后出现问题，可使用回滚功能还原"
  echo -e "- 下载直连失败会自动尝试GitHub加速: ${GITHUB_ACCELERATOR}"
  echo -e "- 当前自动更新版本变体: $(get_variant_choice_display "$AUTO_UPDATE_VARIANT_CHOICE")"
  echo -e "- 快捷命令: smart"
  if is_auto_update_enabled; then
    echo -e "- 自动更新状态: ${GREEN}已启用${NC} (每天 $(get_auto_update_time_display)，$(get_variant_choice_display "$AUTO_UPDATE_VARIANT_CHOICE"))"
  else
    echo -e "- 自动更新状态: ${YELLOW}未启用${NC}"
  fi
  echo -e "- 也可以使用: smart -c 仅查看更新日志"
  echo
...
auto_update() {
  log "开始自动更新检查..."
  log "自动更新版本变体: $(get_variant_choice_display "$UPDATE_VARIANT_CHOICE")"
  detect_arch
  ...
  while [ $# -gt 0 ]; do
    ...
    case "$1" in
      ...
      --help|-h)
        ...
        echo "  --variant-choice <编号>     指定版本变体编号，和手动菜单一致"
        echo "  --variant <编号>            同 --variant-choice"
        echo "  -d, --debug                 显示调试信息"
        echo "  -h, --help                  显示此帮助信息"
      ...
      --variant-choice|--variant)
        ...
        UPDATE_VARIANT_CHOICE="$1"
        ;;
      --variant-choice=*|--variant=*)
        UPDATE_VARIANT_CHOICE="${1#*=}"
        ;;
      *)
        log "警告: 未识别参数 $1，将按自动更新模式执行"
        AUTO_MODE="1"
        RUN_MODE="auto"
        ;;
    esac
